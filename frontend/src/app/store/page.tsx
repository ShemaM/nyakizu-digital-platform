...
import { LandingHeader, LandingFooter, Container } from "@/components/layouts";
        ...
        {linkableStores.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {linkableStores.map((seller) => {
              const storeLabel = seller.store_name || seller.shop_name || usernameOf(seller) || "Store";
              const storeLocation = seller.location || seller.shop_location;
              const categories = (seller.categories ?? []).slice(0, 3);
              return (
                <Link
                  key={seller.id}
